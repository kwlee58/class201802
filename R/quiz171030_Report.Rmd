...
quiz1030 <- read.xlsx("../data/quiz171030.xlsx", 
                      sheetIndex = 1, 
                      startRow = 2, 
                      endRow = 182, 
                      colIndex = c(3:7, 9:14), 
...
names(quiz1030) <- c("dept", "id", "name", "year", "e.mail", "cell.no", "group", "Q1", "Q2", "Q3", "Q4")
quiz1030$dept <- factor(quiz1030$dept)
quiz1030$year <- factor(quiz1030$year)
quiz1030$group <- factor(quiz1030$group, levels = c("Red", "Black"))
A <- which(is.na(quiz1030$Q1))
B <- which(is.na(quiz1030$Q2))
C <- which(is.na(quiz1030$Q3))
D <- which(is.na(quiz1030$Q4))
q3.na <- setdiff(C, B)
table(quiz1030$Q4)
q4.typo <- which(quiz1030$Q4 == "A ")
quiz1030$Q3[q3.na] <- "조퇴"
quiz1030$Q4[q4.typo] <- "A"
quiz1030$Q1 <- factor(quiz1030$Q1, 
                      levels = c("가", "나", "다", "라"), 
                      labels = c("대공황, 대침체", "대침체, 대공황", "대번영, 대공황", "대공황, 대번영"))
quiz1030$Q2 <- factor(quiz1030$Q2, 
                      levels = c("가", "나", "다", "라"), 
                      labels = c("소득불평등, 양의상관", 
                                 "소득, 양의 상관", 
                                 "소득불평등, 인과관계", 
                                 "소득, 인과관계"))
quiz1030$Q3 <- factor(quiz1030$Q3, 
                      levels = c("가", "나", "다", "라"), 
                      labels = c("7.0%", "35%", "70%", "94%"))
quiz1030$Q4 <- factor(quiz1030$Q4, 
                      levels = c("A", "B"))
# quiz1030$Q4.2 <- quiz1030$Q4
# quiz1030$Q4.2[quiz1030$group == "Black"] <- factor(quiz1030$Q4.2[quiz1030$group == "Black"], levels = c("A", "B"), labels = c("100%확률로 잃기", "90%확률로 잃기"))
# quiz1030$Q4.2[quiz1030$group == "Red"] <- factor(quiz1030$Q4.2[quiz1030$group == "Red"], levels = c("A", "B"), labels = c("100%확률로 따기", "90%확률로 따기"))
str(quiz1030)
```

```{r, randomization, echo = FALSE, results = 'hide'}
pander(summary(quiz1030))
```
...
```{r, times, echo = FALSE}
tbl.q1 <- table(quiz1030$group, quiz1030$Q1, exclude = NULL)
...
Q2.tbl.am <- addmargins(table(quiz1030$group, quiz1030$Q2, exclude = NULL), margin = 1)
...
```{r, MTR, echo = FALSE}
tbl.q3 <- table(quiz1030$group, quiz1030$Q3, exclude = NULL)
...
Q4.tbl.e <- table(quiz1030$group, quiz1030$Q4, exclude = NULL)
...
Q4.tbl <- table(quiz1030$group, quiz1030$Q4)
pander(round(prop.table(Q4.tbl, margin = 1)*100, digits = 1))
```

```{r, save, echo = FALSE}
#save.image("../R/quiz1030.rda")
```