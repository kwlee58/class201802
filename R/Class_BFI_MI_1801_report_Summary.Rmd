---
title: "Big Five Personality and Multiple Intelligence : 201801"
author: "coop711"
date: "`r Sys.Date()`"
...
```{r}
BFI.full.kr<-join(class_roll, BFI.level.kr.2, by="ID")
...
(MI.ok.id <- setdiff(1:nrow(class_roll), MI.na.id))
...
```{r}
MI.sort.full.kr<-join(class_roll, MI.sort.kr.2, by="ID")
...
pander(BFI.MI.full.ID[order(BFI.MI.full.ID$ID), ])
```
...
BFI.present<-which(class_roll$ID %in% BFI$ID[BFI.ok.id])
BFI.absent<-(1:nrow(class_roll))[-BFI.present]
BFI.absent
class_roll$ID[BFI.absent]
class_roll$Name[BFI.absent]
```

MI 결석생

```{r, echo = FALSE, results = 'markup'}
MI.present<-which(class_roll$ID %in% MI$ID[MI.ok.id])
MI.absent<-(1:nrow(class_roll))[-MI.present]
MI.absent
class_roll$ID[MI.absent]
class_roll$Name[MI.absent]
```
-->