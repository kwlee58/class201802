...
quiz09 <- read.xlsx("../data/etest09.xlsx", 
                        sheetIndex = 1, 
                        startRow = 3, 
                        endRow = 156, 
                        colIndex = 2:7, 
...
quiz09$days <- as.numeric(difftime("2018-10-22 09:00:00", quiz09$time, units = 'days'))
quiz09$hours <- as.numeric(difftime("2018-10-22 09:00:00", quiz09$time, units = 'hours'))
quiz09$bird <- factor(ifelse(quiz09$hours >= 6, "Early", "Late"), 
...
pander(table(quiz09$group, quiz09$submit))
pander(chisq.test(table(quiz09$group, quiz09$submit)))
```
...
pander(table(cut(quiz09$hours, breaks = breaks, include.lowest = TRUE)), caption = "시간 단위")
```
...
arrows(x0 = 144, y0 = 3, x1 = 160, y1 = 0.65, length = 0.15, code = 2, col = "red", lwd = 1.5)
text(x = 141, y = 3.2, labels = "오후 6시", srt = 15, col = "red")
```
...
```{r, echo = FALSE}
pander(table(quiz09$group, quiz09$bird))
```
...
```{r, echo = FALSE}
pander(t.test(score1 ~ bird, data = quiz09))
```
