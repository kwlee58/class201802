...
TED01 <- read.xlsx("../data/TED180930.xlsx", 
                   sheetIndex = 1, 
                   startRow = 1, 
                   endRow = 154, 
                   colIndex = 1:3, 
...
names(TED01) <- c("name", "score", "time")
TED01$group <- class_roll$group[match(TED01$name, class_roll$name)]
TED01$id <- class_roll$id[match(TED01$name, class_roll$name)]
TED01$submit <- ifelse(is.na(TED01$time), "미제출", "제출")
## 학교 정보시스템이 GMT로 표기되어 있어서 9시간의 타임갭을 감안하여야 함.
TED01$days <- as.numeric(difftime("2018-09-30 09:00:00", TED01$time, units = 'days'))
TED01$bird <- factor(ifelse(TED01$days >= 1, "Early", "Late"), 
                     labels = c("마감날 이전 작성", "마감날 작성"))
str(TED01)
TED01 <- TED01[c("id","score", "time", "group", "submit", "days", "bird")]
str(TED01)
write.table(TED01[c("id", "score", "time")], file = "../data/TE01.txt", row.names = FALSE)
```
