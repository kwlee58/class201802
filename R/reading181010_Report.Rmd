...
reading <- read.xlsx("../data/reading_1801v2.xlsx", 
                     sheetIndex = 1, 
                     startRow = 4, 
                     endRow = 157, 
                     colIndex = c(3:7, 9:48), 
...
reading$group <- factor(reading$group, 
                        levels = c("Red", "Black"))
str(reading$group)
# table(reading$SQ2, exclude = NULL)
table(reading$SQ1)
reading$SQ1 <- factor(reading$SQ1, 
                      levels = c(18:26, 54), 
                      labels = c(18:26, "27세 이상"))
reading$SQ2 <- factor(reading$SQ2, 
                      levels = c(1:2), 
                      labels = c("남", "여"))
# table(reading$group, reading$SQ2,exclude = NULL)
reading$SQ3 <- factor(reading$SQ3, 
                      levels = c(1:17), 
                      labels = c("서울", "부산", "대구", "인천", "광주", "대전", "울산", "경기", "강원", "충북", "충남", "전북", "전남", "경북", "경남", "제주", "세종"))
reading$SQ3 <- droplevels(reading$SQ3)
# leasure.hours.wd <- round((reading$Q1.1.h*60 + reading$Q1.1.m)/60, 
#                           digits = 2)
# leasure.hours.hd <- round((reading$Q1.2.h*60 + reading$Q1.2.m)/60, 
#                           digits = 2)
# reading.hours.wd <- round((reading$Q2.1.h*60 + reading$Q2.1.m)/60, 
#                           digits = 2)
# reading.hours.hd <- round((reading$Q2.2.h*60 + reading$Q2.2.m)/60, 
#                           digits = 2)
# where.1 <- factor(reading$Q3.1, 
#                   levels = 1:6, 
#                   labels = c("집", "직장(학교)", "출퇴근(통학)중", "도서관", "장소불문", "기타"))
# where.2 <- factor(reading$Q3.2, 
#                   levels = 1:6, 
#                   labels = c("집", "직장(학교)", "출퇴근(통학)중", "도서관", "장소불문", "기타"))
# reason.1 <- factor(reading$Q4.1, 
#                    levels = 1:10, 
#                    labels = c("업무", "학업, 취업", "교양, 인격", "지식, 정보", "즐거운 습관", "대화", "시간보내기", "위로와 평안", "실생활도움", "기타"))
# reason.2 <- factor(reading$Q4.2, 
#                    levels = 1:10, 
#                    labels = c("업무", "학업, 취업", "교양, 인격", "지식, 정보", "즐거운 습관", "대화", "시간보내기", "위로와 평안", "실생활도움", "기타"))
# helpful <- factor(reading$Q5, 
#                   levels = 1:5, 
#                   labels = c("매우 도움", "다소 도움", "보통", "별 도움 안됨", "매우 도움 안됨"))
obstacles <- factor(reading$Q6.1, 
                    levels = 1:12, 
                    labels = c("일(공부) 때문", "싫고 습관화 안됨", "다른 여가활동", "몸 불편", "책 고르기 난감", "읽을 만한 책 없음", "장소 없음", "경제적 여유 없음", "독해 난감", "필요성 못 느낌", "마음의 여유 없음", "기타"))
...
reader.p.f <- factor(reader.p, 
                     levels = c("Yes", "No"), 
                     labels = c("1권 이상 읽는다", "1권도 읽지 않는다"))
reader.e <- ifelse(reading$Q7.E >= 1, "Yes", "No")
reader.e.f <- factor(reader.e, 
                     levels = c("Yes", "No"), 
                     labels = c("1권 이상 읽는다", "1권도 읽지 않는다"))
enough.f <- factor(reading$Q8, 
                   labels = c("매우 충분", "충분", "보통", "부족", "매우 부족"))
...
tbl_SQ1 <- table(reading$group, reading$SQ1, exclude = NULL)[-3, ]
colnames(tbl_SQ1)[11] <- "결석"
pander(tbl_SQ1, caption = "나이")
tbl_SQ2 <- table(reading$group, reading$SQ2, exclude = NULL)[-3, ]
colnames(tbl_SQ2)[3] <- "결석"
pander(tbl_SQ2, caption = "성별")
tbl_SQ3 <- table(reading$group, reading$SQ3, exclude = NULL)[-3, ]
colnames(tbl_SQ3)[10] <- "결석"
pander(tbl_SQ3, caption = "지역")
```
...
```{r, leasure hrs work days, echo = FALSE, eval = FALSE}
...
```{r, leasure hrs holidays, echo = FALSE, eval = FALSE}
...
```{r, reading hrs work days, echo = FALSE, eval = FALSE}
...
```{r, reading hrs holidays, echo = FALSE, eval = FALSE}
...
```{r, reading location, echo = FALSE, eval = FALSE}
...
```{r, reason to read, echo = FALSE, eval = FALSE}
...
```{r, how helpful, echo = FALSE, eval = FALSE}
...
pander(table(reading$group, reader.p.f), caption = "종이책(명)")
pander(prop.table(table(reader.p.f))*100, caption = "종이책(%)", digits = 3)
pander(table(reading$group, reader.e.f), caption = "전자책(명)")
pander(prop.table(table(reader.e.f))*100, caption = "전자책(%)", digits = 3)
```
...
```{r, advice, echo = FALSE, eval = FALSE}
...
pander(area.p.df[most.read.ind, ])
```
