...
literacy_roll <- read.xlsx("../data/literacy_1802.xlsx", 
                           sheetIndex = 1, 
                           startRow = 3, 
                           endRow  = 158, 
                           colIndex = c(3:7, 9:35), 
...
names(literacy_roll) <- c("dept", "id", "name", "year", "e.mail", "cell.no", 
                          paste0("Q", 1:25), "group")
literacy_roll$group %<>% 
  factor(levels = c("Red", "Black")) #> Boxplot 에서 `xlim` = finite 오류를 피하려면 `factor`변환 필요
literacy_roll$group %>%
  table
## 수강신청 변경 기간에 Red and Black 부여가 Black 에 치우친 결과 보정
## 문해력조사 이후 수강신청변경으로 출석부에서 빠진 사람들 파악. `Black`이 다소 많이 배정됨.
## 9월19일자 퀴즈 자료를 비교 대상으로 추출
quiz0919 <- readRDS("quiz180919.RDS")
literacy_roll %>%
#  `[`(.$id %in% setdiff(.$id, quiz0919$id), c("id", "name", "group"))
#  `[`(`%in%`(.$id, setdiff(.$id, quiz0919$id)), c("id", "name", "group"))
  `[`(`%in%`(.$id, .$id %>% setdiff(quiz0919$id)), c("id", "name", "group"))
## 
literacy_roll %<>%
  `[<-`(.$id %in% c("20172840", "20183968"), "group", "Red") 
## `[<-` 를 이용하여 값을 바꾸는 것은 다음 코드를 수행하는 것과 동등하다.
# literacy_roll[literacy_roll$id %in% c("20172840", "20183968"), "group"]  <- "Red"
literacy <- literacy_roll[, c(7:32)] #> 문해력 집계에 필요한 변수만 추출. 
kable(literacy)
```
...
par(family = "KoPubDotum Medium", mar = c(5.1, 4.1, 4.1, 8.1))
...
<!--* 정답 맞춰보기
    * 정답 읽어들이기. `what = "KoPubDotum Medium"`로 설정하여 `char`취급. 
