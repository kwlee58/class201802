...
literacy.roll <- read.xlsx("../data/literacy_1702.xlsx", 
                           sheetIndex = 1, 
                           startRow = 3, 
                           endRow  = 182, 
                           colIndex = c(3:7, 9:34), 
...
names(literacy.roll) <- c("dept", "id", "name", "year", "e.mail", "cell.no", paste("Q", 1:25, sep = ""))
...
str(literacy.roll)
class_roll <- readRDS("./class_roll0908.RDs")
str(class_roll)
literacy <- merge(literacy.roll, class_roll, 
                  by = c("name", "dept", "id", "year", "e.mail"), 
                  all = TRUE)
#> 김보성, 김윤재, 조민연, 최동규, 현진민 학생의 전화번호는 하이픈 바짐. 박시훈은 전화번호 바뀜
str(literacy)
kable(literacy)
class_roll <- literacy[, c("dept", "id", "name", "year", "e.mail", "cell.no.x", "group")]
kable(class_roll)
...
literacy <- literacy[, c(7:31, 33)] #> 문해력 집계에 필요한 변수만 추출. 
kable(literacy)
...
save.image("literacy_201702.RData")
```
-->
