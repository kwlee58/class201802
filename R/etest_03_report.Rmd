...
names(quiz03) <- c("name", "id", "dept", "score1", "score2", "time")
quiz03$group <- class_roll$group[match(quiz03$id, class_roll$id)]
...
pander(table(quiz03$group, quiz03$submit))
...
```{r, echo = FALSE}
pander(table(quiz03$group, quiz03$bird))
```
...
```{r, echo = FALSE}
pander(t.test(score1 ~ bird, data = quiz03))
```

## Save

```{r, echo = FALSE}
saveRDS(quiz03, file = "quiz03.RDs")
```
