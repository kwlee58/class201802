...
names(quiz04) <- c("name", "id", "dept", "score1", "score2", "time")
quiz04$group <- class_roll$group[match(quiz04$id, class_roll$id)]
...
pander(table(quiz04$group, quiz04$submit))
...
```{r, echo = FALSE}
pander(table(quiz04$group, quiz04$bird))
...
```{r, echo = FALSE}
pander(t.test(score1 ~ bird, data = quiz04))
```

## Save

```{r, echo = FALSE}
saveRDS(quiz04, file = "quiz04.RDs")
```