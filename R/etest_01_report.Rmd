...
id_stay <- intersect(id0901, id0907)
...
pander(table(quiz01$group, quiz01$submit))
```
...
pander(table(cut(quiz01$days, breaks = c(0:7), include.lowest = TRUE)), caption = "일 단위")
...
```{r, echo = FALSE}
pander(table(quiz01$group, quiz01$bird))
```
...
```{r, echo = FALSE}
saveRDS(quiz01, file = "quiz01.RDs")
```