...
names(quiz02) <- c("name", "id", "dept", "score1", "score2", "time")
quiz02$group <- class_roll$group
...
pander(table(quiz02$group, quiz02$submit))
...
```{r, echo = FALSE}
pander(table(quiz02$group, quiz02$bird))
```
...
```{r, echo = FALSE}
pander(t.test(score1 ~ bird, data = quiz02))
```

## Save

```{r, echo = FALSE}
saveRDS(quiz02, file = "quiz02.RDs")
```