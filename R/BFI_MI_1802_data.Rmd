...
library(xlsx)
options(width = 180)
class_roll <- readRDS("class_roll.RDS")
class_roll <- class_roll[c("id", "group")]
class_roll %<>%
  `names<-`(c("ID", "Group"))
BFI <- "../data/BFI181010.xlsx" %>% 
...
class_roll <- class_roll %>%
  subset(class_roll$ID %in% BFI$ID)
```

### Save Data

```{r, save.image}
save.image("./BFI_MI_1802.RData")
```
