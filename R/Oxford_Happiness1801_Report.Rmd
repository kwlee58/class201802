...
oxford <- "../data/oxford_happiness180425.xlsx" %>% 
  read.xlsx(sheetIndex = 1, 
            startRow = 3, 
            endRow = 182, 
            colIndex = c(4:5, 10:41), 
            colClasses = c(rep("character", 2), rep("numeric", 31), "character"),
            header = FALSE, 
            stringsAsFactors = FALSE)
names(oxford) <- c("ID", "Name", paste0("Q", 1:29), "Scores_M", "Duration", "Group")
oxford$Group %<>% factor(levels = c("Red", "Black"), 
                         labels = c("국문", "영문"))
#> (역) 또는 (R) 항목은 7에서 뺀 점수로 
oxford[, paste0("Q", c(1, 5, 6, 10, 13, 14, 24, 27:29))] %<>% 
  `-` (7, .)
oxford$Scores <- oxford[ , 3:31] %>% apply(MARGIN = 1, FUN = function(x) sum(x) / 29)
...
```{r, fig.width = 6.5, fig.height = 6.5, echo = FALSE, eval = TRUE}
par(family = "HCR Dotum LVT")
...
```{r, echo = FALSE}
oxford %$% Diff %>% tapply(INDEX = oxford$Group, FUN = t.test) %>% pander
```
