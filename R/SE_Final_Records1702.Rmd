...
total <- read.xlsx(file = "../data/SE_scoreList_1801.xlsx", 
                   sheetIndex = 1, 
                   rowIndex = 3:48, 
                   colIndex = 2:7, 
                   colClasses = c(rep("character", 2), rep("numeric", 6)), 
                   header = TRUE)
total <- total[c(2, 1, 5, 4, 6, 3)]
v.names.total <- c("ID", "Name", "Offline", "Online", "Match Maker", "Total")
names(total) <- v.names.total
N <- nrow(total)
total$Ranks <- N + 1 - rank(total$Total)
total$Grades <- ifelse(total$Offlinel == 0, "Not Pass", "Pass")
kable(total)
```
...
v.names.kr <- c("학번", "이름", "오프라인출석(10)", "온라인출석(10)", "매치메이커", "총점(100)", "학점")
total.kr <- total
