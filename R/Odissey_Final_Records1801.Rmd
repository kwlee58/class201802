...
total <- read.xlsx(file = "../data/Odissey_scoreList_1801.xlsx", 
                   sheetIndex = 1, 
                   rowIndex = 3:28, 
                   colIndex = 2:6, 
                   colClasses = c(rep("character", 2), rep("numeric", 6)), 
                   header = TRUE)
total <- total[c(2, 1, 4, 5, 3)]
v.names.total <- c("ID", "Name", "Assignment", "Counsel", "Total")
names(total) <- v.names.total
N <- nrow(total)
total$Ranks <- N + 1 - rank(total$Total)
total$Grades <- ifelse(total$Total == 0, "Not Pass", "Pass")
kable(total)
```
...
v.names.kr <- c("학번", "이름", "과제", "면담", "총점(100)", "학점")
total.kr <- total
